...
interface DraggingSignature extends Signature {
  offsetX: number;
  offsetY: number;
  imgWidth: number;
  imgHeight: number;
...
interface RotatingSignature extends Signature {
  pageIndex: number;
}
...
export default function PdfSignTool() {
  ...
  const [draggingSignature, setDraggingSignature] = useState<DraggingSignature | null>(null);
  const [resizingSignature, setResizingSignature] = useState<ResizingSignature | null>(null);
  const [stretchingSignature, setStretchingSignature] = useState<StretchingSignature | null>(null);
  const [rotatingSignature, setRotatingSignature] = useState<RotatingSignature | null>(null);
  ...
  const [editingSignatureColor, setEditingSignatureColor] = useState<number | null>(null);
  ...
  const getPixelPositions = useCallback((sig: Signature, pageIndex: number) => {
  ...
  useEffect(() => {
    const loadFonts = async () => {
      const fontFaces = SIGNATURE_FONTS.map((font) => {
  ...
  useEffect(() => {
    if (!resizingSignature && !rotatingSignature && !stretchingSignature) return;

    const handleMouseMove = (e: MouseEvent) => {
      // Resize (proportional)
      if (resizingSignature) {
        const img = imageRefs.current[resizingSignature.pageIndex];
        if (!img) return;

        const dx = e.clientX - resizingSignature.startX;
        const dy = e.clientY - resizingSignature.startY;
        const imgWidth = img.clientWidth;
        const imgHeight = img.clientHeight;

        let deltaPercent = 0;

        switch (resizingSignature.corner) {
          case 'se':
        ...
        const newWidthPercent = Math.max(0.05, Math.min(0.9, resizingSignature.startWidthPercent + deltaPercent));
        const aspectRatio = resizingSignature.startWidthPercent / resizingSignature.startHeightPercent;
      ...
      // Stretch (independent)
      if (stretchingSignature) {
        const img = imageRefs.current[stretchingSignature.pageIndex];
        if (!img) return;

        const dx = e.clientX - stretchingSignature.startX;
        const dy = e.clientY - stretchingSignature.startY;
        const imgWidth = img.clientWidth;
        const imgHeight = img.clientHeight;

        let newWidthPercent = stretchingSignature.startWidthPercent;
        let newHeightPercent = stretchingSignature.startHeightPercent;
        let newXPercent = stretchingSignature.startXPercent;
        let newYPercent = stretchingSignature.startYPercent;

        switch (stretchingSignature.edge) {
          case 'e':
            newWidthPercent = Math.max(0.05, stretchingSignature.startWidthPercent + dx / imgWidth);
            break;
          case 'w':
            const deltaW = dx / imgWidth;
            newWidthPercent = Math.max(0.05, stretchingSignature.startWidthPercent - deltaW);
            newXPercent = stretchingSignature.startXPercent + deltaW;
            break;
          case 's':
            newHeightPercent = Math.max(0.05, stretchingSignature.startHeightPercent + dy / imgHeight);
            break;
          case 'n':
            const deltaH = dy / imgHeight;
            newHeightPercent = Math.max(0.05, stretchingSignature.startHeightPercent - deltaH);
            newYPercent = stretchingSignature.startYPercent + deltaH;
            break;
        }

        setSignatures(prev =>
          prev.map(sig =>
            sig.id === stretchingSignature.id
      ...
      if (rotatingSignature) {
        const img = imageRefs.current[rotatingSignature.pageIndex];
        if (!img) return;

        const rect = img.getBoundingClientRect();
        const pixelPos = getPixelPositions(rotatingSignature, rotatingSignature.pageIndex);
    ...
    const handleMouseUp = () => {
      ...
      setStretchingSignature(null);
      setRotatingSignature(null);
    };
  ...
  }, [resizingSignature, rotatingSignature, stretchingSignature, getPixelPositions]);
  ...
  const stopDrawing = () => {
    if (isDrawing && drawCanvasRef.current) {
      setDrawnSignature(drawCanvasRef.current.toDataURL());
    }
    setIsDrawing(false);
  };
  ...
  const addSignatureToPage = async () => {
    ...
    const newSignature: Signature = {
      ...
      content,
      font: selectedFont,
      fontName: SIGNATURE_FONTS[selectedFont].name,
  ...
  const startDrag = (sig: Signature, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    setDraggingSignature({ ...sig, offsetX, offsetY, imgWidth: img.clientWidth, imgHeight: img.clientHeight });
  };
  ...
  const startRotate = (sig: Signature, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setRotatingSignature({ ...sig, pageIndex: sig.pageIndex });
  };

  const onMouseMove = (e: React.MouseEvent) => {
    if (draggingSignature) {
      let targetPageIndex = draggingSignature.pageIndex;
      ...
      const newX = e.clientX - rect.left - draggingSignature.offsetX;
      const newY = e.clientY - rect.top - draggingSignature.offsetY;

      const newXPercent = Math.max(0, Math.min(1 - draggingSignature.widthPercent, newX / imgWidth));
      const newYPercent = Math.max(0, Math.min(1 - draggingSignature.heightPercent, newY / imgHeight));

      setSignatures(signatures.map(sig =>
        sig.id === draggingSignature.id
          ? { ...sig, xPercent: newXPercent, yPercent: newYPercent, pageIndex: targetPageIndex }
          : sig
      ));

      if (targetPageIndex !== draggingSignature.pageIndex) {
        setDraggingSignature({ ...draggingSignature, pageIndex: targetPageIndex });
      }
    } else if (rotatingSignature) {
      const img = imageRefs.current[rotatingSignature.pageIndex];
      if (!img) return;
      ...
      const angle = Math.atan2(mouseY - centerY, mouseX - centerX) * (180 / Math.PI);
      setSignatures(signatures.map(sig =>
        sig.id === rotatingSignature.id ? { ...sig, rotation: angle } : sig
      ));
    }
  };
  ...
  const updateSignatureColor = (id: number, color: string) => {
    setSignatures(signatures.map(sig =>
  ...
  const downloadSignedPdf = async () => {
    ...
    try {
      ...
      for (const sig of signatures) {
        ...
        if (sig.type === "type") {
          if (!fontCache[sig.font]) {
            const fontUrl = SIGNATURE_FONTS[sig.font].url;
      ...
      alert("Failed to sign PDF: " + (err as Error).message);
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div className="flex flex-col items-center w-full pt-2 pb-8 px-4">
      <div className="max-w-[1200px] w-full flex flex-col gap-4">
        ...
        <div className="text-center">
          ...
          <h1 className="text-xl font-black tracking-tight mt-2">Sign PDF</h1>
          <p className="text-[13px] text-gray-500 dark:text-gray-400 mt-1">Add your signature to PDF documents</p>
        </div>
            ...
            <div className="flex flex-col gap-4 h-[800px] overflow-visible pr-2">
              ...
              <div className="flex gap-2">
                {[{ m: "type", l: "Type" }, { m: "draw", l: "Draw" }, { m: "upload", l: "Upload" }].map(({ m, l }) => (
                  <button key={m} onClick={() => setMode(m as SignatureMode)}
              ...
              {mode === "type" && (
                <div className="flex flex-col gap-3">
                  <input type="text" value={signatureName} onChange={(e) => setSignatureName(e.target.value)}
                    placeholder="Enter your name" className="w-full px-4 py-2.5 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-sm focus:ring-2 focus:ring-black dark:focus:ring-white outline-none" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="flex gap-2 items-center">
                        <div className="relative flex-1">
                          ...
                          >
                            ...
                            ></div>
                            <span className="text-xs font-mono">{signatureColor}</span>
                          </button>
                          {showColorPicker && (
                    ...
                    {signatureName && (
                      <div className="col-span-1 md:col-span-2 p-11 bg-gray-50 dark:bg-neutral-800 rounded-lg border border-gray-100 dark:border-neutral-800 flex justify-center items-center relative">
                        ...
                        <div
                          style={{
                            fontFamily: fontsLoaded ? `"${SIGNATURE_FONTS[selectedFont].name}", cursive` : 'cursive',
                            color: signatureColor,
              ...
              {mode === "draw" && (
                <div className="flex flex-col gap-3">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {colorPresets.map(color => (
                        <button
                          ...
                          className={`w-8 h-8 rounded-md border-2 hover:scale-110 transition-transform ${signatureColor === color ? 'border-black dark:border-white' : 'border-gray-300 dark:border-neutral-600'}`}
                          style={{ backgroundColor: color }}
                        />
                      ))}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-gray-600 dark:text-gray-400 mb-2">Draw Your Signature</label>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {uploadedImage && (
                    <div className="p-4 bg-gray-50 dark:bg-neutral-800 rounded-lg border border-gray-100 dark:border-neutral-800">
                      ...
                      <img src={uploadedImage} alt="Signature" className="max-w-full h-auto rounded" />
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="flex flex-col gap-3 h-[800px]">
              {!file ? (
                ...
                <>
                    ...
                    {pdfPages.map((page, pageIndex) => (
                      ...
                      >
                        ...
                        <img
                          ...
                          draggable={false}
                        />
                        {signatures.filter(sig => sig.pageIndex === pageIndex).map(sig => {
                          return (
                  ...
                  <button onClick={downloadSignedPdf} disabled={processing || signatures.length === 0}
                    className={`w-full py-2.5 rounded-lg text-xs font-bold transition-all ${processing || signatures.length === 0 ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-blue-600 text-white hover:bg-blue-700 shadow-sm"}`}>
                    {processing ? "Processing..." : "Download Signed PDF"}
                  </button>
                </>
              )}
            </div>
